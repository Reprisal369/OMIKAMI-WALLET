...
DEPENDABOT TRIAGE + RPC PINNING → v0.5.2 (2026-07-26). Triaged the two open Dependabot PRs. npm-minor (#6, dev-only: playwright/eslint/typescript-eslint) = safe. npm-patch (#11) was RED: the **bundle gate correctly caught a new host** — `viem 2.55.10` changed the mainnet default RPC from `eth.merkle.io` to `ethereum.reth.rs`, a real egress change (used for ENS). Chose Option A (owner): **pin our own transports** in `wagmi.ts` (Sepolia default → thirdweb, mainnet/ENS → eth.merkle.io) so viem default changes can never silently move our egress; added `reth.rs` to the bundle allowlist as a bundled-but-never-contacted constant (documented). Verified BOTH ways: current viem 2.55.1 all gates green (37 files); and with viem 2.55.10 the bundle gate now passes (36 files, reth present-but-allowlisted, eth.merkle.io still the pinned runtime host). This unblocks Dependabot #11/#6 to merge (rebased) with egress unchanged. Version → 0.5.2, CHANGELOG added. No new features; still read-only.

TEST COVERAGE hardening (test-only, no product code, no version bump): added security-relevant edge cases → unit **86 → 93**. `rpc.test.ts` +4 (malformed/not-a-url, public IPv6 fails-closed L3/I4, case-insensitive internal host + port strip, public host with port + path/query normalization) = 14; `allowance.test.ts` +1 (exact unlimited-threshold boundary) = 12; token-registry `formatTokenAmount` +2 (trailing-zero trim, exact-6 vs 7-digit ≈ marker) = 13. All green: lint · typecheck · forbidden · secrets.
